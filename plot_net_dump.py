...
def main(args=None):
    ...
    optParser.add_option("--fw", dest="figwidth", default="10", help="figure width")
    optParser.add_option("--fh", dest="figheight", default="10", help="figure height")
    ...
    for t in times:
        ...
        figwidth = getattr(options, "figwidth", 10)
        figheight = getattr(options, "figheight", 8)

        fig, ax = helpers.openFigure(options)
        fig.set_size_inches(float(figwidth), float(figheight))


        ax.set_aspect("equal", None, 'C')
        helpers.plotNet(net, colors, widths, options)

        #plt.figure(figsize=(10, 10))  # Resize the figure
        ...
        color_bar = plt.colorbar(sm, orientation='horizontal', pad=0.1 , fraction=0.05 )

        #color_bar.set_label(options.colorBarLabel)
        color_bar.set_label('CO2 [mg]')
        ...
        #plt.savefig("reports/imagenes/mapco2.png", dpi=300, bbox_inches='tight')
        #plt.close()
    return 0
