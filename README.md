...
- **Datos folder:** Archivo xls con datos de tráfico.
- **Outputs folder:** Folder donde se almacenan resultados.  

Su ejecución viene dada del siguiente modo:
```bash
./emissions.sh <día> <datos folder> <output folder>
```

Ejemplo de Uso:
```bash
./emissions.sh lunes datosxlsx outputs
```
...
Crea la matriz O/D de tráfico en el formato V (VISUM/VISSIM) entendible para SUMO. Se genera un .od file por cada hora de simulación. Requiere un archivo de entrada llamado "**traffic_hour_weights.xlsx**" que contiene el peso para cada hora de tráfico. Este archivo se ubica dentro del folder **datosxlsx**. Esto se considera dado que los datos de entrada son totales para 12 horas.   

**_Funcion solo para 12 horas  - Pendiente update_**

Para ejecutar el script tenemos los siguientes parámetros:


```bash
python3  genOD_files.py <traffic_.xlxs> <datosxlxs/lunes09-12.xlsx> <OD output folder> <tiempo_inicio_simulacion> <tiempo_final_simulacion> <factor>
```

Los parámetros de entrada para la ejecución de este script viene dado por lo siguiente:
1. 
2. Nombre del archivo Excel (<traffic_.xlxs>).
2. Carpeta donde se guardan los formatos V (.od) (<archivo_salida.od>).
3. Tiempo de inicio **formato 24hr**. (<tiempo_inicio_simulacion>).
4. Tiempo de final **formato 24hr**(<tiempo_final_simulacion>).
5. Un número adicional (factor de escalamiento). **No usado**. 
...
El archivo de trafico xlxs proporcionado muestra trafico total de e.j., 12 horas. No distinge trafico por hora. El trafico de 12 horas se genera con pesos para cada hora para generar un trafico realista y evitar congestiones.  
